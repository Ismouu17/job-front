            ...
            {application.jobApplicant.education
              .map((edu) => {
                return `${edu.institutionName} (${edu.startYear}-${
                  edu.endYear ? edu.endYear : 'En cours'
                })`
              })
              .join(', ')}
          </Grid>
...
const JobApplications = (props) => {
  ...
  const getData = () => {
    ...
    const jobIds = JSON.stringify(jobId).split(':')[1].split('}')[0]
    let address = `${apiList.applicants}?jobId=${jobIds.replace(/"/g, '')}`
    if (queryString !== '') {
